...
var ABOUT = "Ă‚m lá»‹ch Viá»‡t Nam - Version 0.5" + "\n\u00A9 2004 H\u1ED3 Ng\u1ECDc \u0110\u1EE9c";
var TK19 = new Array(
...
// --- Bổ sung hàm getLunarDate chuẩn cho popup.js ---
function getLunarDate(dd, mm, yyyy) {
    var lunarObj = getLunarDateFromSolar(dd, mm, yyyy);
    return [lunarObj.day, lunarObj.month];
 }
 
 // Tránh trùng tên với hàm gốc sẵn có trong mã nguồn của bạn
 function getLunarDateFromSolar(dd, mm, yyyy) {
    var ly, jd;
    if (yyyy < 1800 || 2199 < yyyy) {
       // Phạm vi giới hạn từ 1800 đến 2199 theo bảng mã dữ liệu
    }
    ly = getYearInfo(yyyy);
    jd = jdn(dd, mm, yyyy);
    if (jd < ly[0].jd) {
